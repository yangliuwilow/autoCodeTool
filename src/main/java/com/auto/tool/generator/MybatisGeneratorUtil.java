...
public class MybatisGeneratorUtil {
    ...
    // ServiceImpl模板路径
    private static String Api_vm = "/template/Api.vm";

    // ServiceImpl模板路径
    private static String ApiImpl_vm = "/template/ApiImpl.vm";
    ...
    public static void generator(
        ...
        try {
            ...
            for (Map map : result) {
                logger.info("table_name:{}", map.get("TABLE_NAME") != null ? map.get("TABLE_NAME").toString() : "");
                table = new HashMap<String, Object>(2);
                table.put("table_name", map.get("TABLE_NAME") != null ? map.get("TABLE_NAME").toString() : "");
                table.put("model_name", StringUtil.lineToHump(ObjectUtils.toString(map.get("TABLE_NAME") != null ? map.get("TABLE_NAME").toString() : "").replace("tb_","")));
                tables.add(table);
            }
            jdbcUtil.release();
            ...
            context.put("jdbcDriver",jdbcDriver);
            context.put("jdbcUrl",jdbcUrl);
            context.put("jdbcUsername",jdbcUsername);
            context.put("jdbcPassword",jdbcPassword);
        ...
        String ApiPath = basePath + module + "/" + module + "-rpc-api" + "/src/main/java/" + packageName.replaceAll("\\.", "/") + "/api";
        String ApiImplPath = basePath + module + "/" + module + "-rpc-service" + "/src/main/java/" + packageName.replaceAll("\\.", "/") + "/api/impl";

        String BusinessPath = basePath + module + "/" + module + "-rpc-api" + "/src/main/java/" + packageName.replaceAll("\\.", "/") + "/business/service";
        String BusinessImplPath = basePath + module + "/" + module + "-rpc-service" + "/src/main/java/" + packageName.replaceAll("\\.", "/") + "/business/service/impl";


        String BusinessApiPath = basePath + module + "/" + module + "-rpc-api" + "/src/main/java/" + packageName.replaceAll("\\.", "/") + "/business/api";
        String BusinessApiImplPath = basePath + module + "/" + module + "-rpc-service" + "/src/main/java/" + packageName.replaceAll("\\.", "/") + "/business/api/impl";
        ...
        createDir(new File(serviceImplPath));
        createDir(new File(ApiPath));
        createDir(new File(ApiImplPath));
        createDir(new File(BusinessPath));
        createDir(new File(BusinessImplPath));

        createDir(new File(BusinessApiPath));
        createDir(new File(BusinessApiImplPath));
        //  TODO 自己修改jar
        List<IntrospectedTable> IntrospectedTables = myBatisGenerator.configuration.getContexts().get(0).introspectedTables;

        for (int i = 0; i < tables.size(); i++) {
            String model = StringUtil.lineToHump(ObjectUtils.toString(tables.get(i).get("table_name").toString().replace("tb_","")));
            String service = servicePath + "/" + model + "Service.java";
            String serviceImpl = serviceImplPath + "/" + model + "ServiceImpl.java";
            String mapperFile = mapperPath + "/" + model + "Mapper.java";
            String Api = ApiPath + "/" + model + "Api.java";
            String ApiImpl = ApiImplPath + "/" + model + "ApiImpl.java";

            String BusinessServerPath = BusinessPath + "/" + model + "BusinessService.java";
            String BusinessServerImplPath = BusinessImplPath + "/" + model + "BusinessServiceImpl.java";

            String BusinessPathApi = BusinessApiPath + "/" + model + "BusinessApi.java";
            String BusinessPathApiImpl = BusinessApiImplPath + "/" + model + "BusinessApiImpl.java";
            ...
            // 生成 api
            VelocityUtil.generate(Api_vm, Api, context);
            // 生成 ApiImpl
            VelocityUtil.generate(ApiImpl_vm, ApiImpl, context);

            // 生成 api
            VelocityUtil.generate("/template/BusinessService.vm", BusinessServerPath, context);
            // 生成 ApiImpl
            VelocityUtil.generate("/template/BusinessServiceImpl.vm", BusinessServerImplPath, context);
            logger.info(controller);

            // 生成 api
            VelocityUtil.generate("/template/BusinessApi.vm", BusinessPathApi, context);
            // 生成 ApiImpl
            VelocityUtil.generate("/template/BusinessApiImpl.vm", BusinessPathApiImpl, context);
